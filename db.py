...
def save_transaction(amount, type, category, note, date):
    ...
    con = sqlite3.connect("database.db")
    cur = con.cursor()
    cur.execute('''INSERT INTO transactions(amount, type, category, note, date)
                   VALUES (?, ?, ?, ?, ?)''', (amount, type, category, note, date))
    con.commit()
    con.close()
...
def show_by(n, mode):
    """
    Filter transactions by date (n=1) or category (n=2).
    """
    con = sqlite3.connect('database.db')
    cur = con.cursor()
...
def edit(trans, n):
    ...
    cur.execute('''UPDATE transactions
                   ...
                   WHERE id = ?''', (trans[0], trans[1], trans[2], trans[3], trans[4], n))
    con.commit()
    con.close()
...
def draw():
    """
    Draw a pie chart of income vs expense and save it to the device.
    """
