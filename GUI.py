...
def table(window):
    ...
    test=[(1,'2025',1200,'income','gift','br gift'),(2,'2025',300,'expanse','food','lunch')]
...
def show_form():
    window= ttk.Toplevel()
    window.title('All transaction')
    window.geometry('960x400')
    window.resizable(False,False)
    table(window)
    btn=ttk.Button(window,text='new transaction',command=open_add_transaction_form,width=30,bootstyle="primary_outline")
    btn.pack(pady=10)
    window.mainloop()
...
def balance_form():
    ...
    bl=db.show_balance()
    lb=ttk.Label(form,text=f'Your current Balane is:{bl}',font=("Helvetica", 15),justify='left')
...
def showby_form():
    ...
    combo=ttk.Combobox(form, values=["Date", "Category"], width=28, state="readonly")
    combo.pack()
    ...
    def sshow(n,mode):        
        ...
        if not result:
            ...
            btn=ttk.Button(form,text='new transaction',command=open_add_transaction_form,width=30,bootstyle="primary_outline")
            btn.pack(pady=10)
            form.mainloop()
...
def edit_form():
    ...
    index_entry.pack()
    ttk.Button(form, text="Confrim", command=new_edit,bootstyle="primary_outline").pack(pady=15)
...
def delete_form():
    ...
    index_entry=ttk.Entry(form,width=30)
    index_entry.pack()
    btn=ttk.Button(form,text='Delete',command= lambda:(messagebox.showinfo('Deleted Successfully' if delete_transaction(index_entry.get()) else 'Something went wrong')))
    btn.pack()
...
def export_form():
    ...
    messagebox.showinfo('SUCCESS',"Exported successfully in: /storage/emulated/0/Documents/Transaction/export.csv")
...
def report_form():
    ...
    ttk.Label(form, text=f"Balane :{record[2]}", font=("Helvetica", 11)).pack(pady=5)
    ttk.Label(form, text=f"Total By Category:{record[3]}", font=("Helvetica", 11)).pack(pady=5)
    ttk.Label(form, text=f"Transation Count By Date:{record[4]}", font=("Helvetica", 11)).pack(pady=5)
...
if __name__ == "__main__":
    ...
    ttk.Button(app, text='Show all transaction',width=25, command =show_form, bootstyle='primary_outline').pack(pady=5)
    ttk.Button(app, text='Show balance',width=25, command=balance_form, bootstyle='primary_outline').pack(pady=5)
    ttk.Button(app, text='Show by (date ot category)',width=25, command=showby_form, bootstyle='primary_outline').pack(pady=5)
